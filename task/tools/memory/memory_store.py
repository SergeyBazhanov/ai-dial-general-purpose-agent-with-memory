...
class LongTermMemoryStore:
    ...
    async def _get_memory_file_path(self, dial_client: AsyncDial) -> str:
        ...
        return f"files/{app_home.as_posix()}/__long-memories/data.json"
    ...
    async def _save_memories(self, api_key: str, memories: MemoryCollection):
        ...
        await dial_client.files.upload(url=memory_file_path, file=json_str.encode('utf-8'))
    ...
    def _deduplicate_fast(self, memories: list[Memory]) -> list[Memory]:
        ...
        for i in range(len(memories)):
            if i in to_remove:
                continue
            for j_pos in range(1, k):  # Skip index 0 (self-match)
                j = int(indices[i][j_pos])
                if j in to_remove:
                    continue
                if similarities[i][j_pos] > 0.75:
